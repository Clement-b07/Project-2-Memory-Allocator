...
void *do_alloc(size_t size) {
        ...
        return (void*)((intptr_t)head + sizeof(header));
...
void *tumalloc(size_t size) {
    ...
    // Find the best fit block (smallest that is big enough)
    while (curr) {
        if (size <= curr->size) {
    ...
    if (best_fit != NULL) {
        ...
        void *ptr = split(best_fit, size);
        if (ptr != NULL) {
            if (best_prev == NULL) {

                // best_fit is the HEAD
                HEAD = best_fit->next;
            } else {
...
void *tucalloc(size_t num, size_t size) {
    ...
    if (num == 0 || size == 0) {
        return NULL;    // Avoid zero-sized allocation
    }

    size_t total_size = num * size; // Calculate total memory required
    ...
    void *ptr = tumalloc(total_size); // Allocate memory using tumalloc
    if (ptr != NULL) {
...
void *turealloc(void *ptr, size_t new_size) {
    ...
    if (new_size == 0) {
        ...
        return NULL; // If new_size is 0, return NULL (or free memory if needed)
    }

    if (ptr == NULL) {

        return tumalloc(new_size); // If ptr is NULL, just allocate new memory
    }
    ...
    // Optionally, the old block could be added to the free list
    return new_ptr;
}
